...
var require_readdir_glob = __commonJS({
  "node_modules/readdir-glob/index.js"(exports, module2) {
      ...
      for (const file of files) {
        ...
        if (stats.isDirectory()) {
          if (!shouldSkip(relative)) {
            ...
            yield* exploreWalkAsync(filename, path2, followSyslinks, useStat, shouldSkip, false);
          }
        } else {
          yield { relative, absolute, stats };
        }
    ...
    async function* explore(path2, followSyslinks, useStat, shouldSkip) {
      yield* exploreWalkAsync("", path2, followSyslinks, useStat, shouldSkip, true);
    }
...
var parseFile = (file) => {
  return new Set(file.replace(/#.+/gm, "").replace(/^\r\n/gm, "").replace(/\r\n$/, "").split(/\r\n/gm).map((v) => v.split(/,\s+/gm)));
};
var convertDic = (fileOrFolderPath) => {
  const sett = /* @__PURE__ */ new Set();
  const pathState = import_fs.default.statSync(fileOrFolderPath);
  const isDirectory = pathState.isDirectory();
  if (isDirectory) {
    import_fs.default.readdirSync(fileOrFolderPath).forEach((p) => {
      const set = convertDic(import_path.default.join(fileOrFolderPath, p));
      set.forEach((v) => {
        sett.add(v);
      });
    });
  } else {
    const file = import_fs.default.readFileSync(fileOrFolderPath, { encoding: "utf-8" });
...
var createDic = (distPath, dicPath) => {
  const zip = (0, import_archiver.default)("zip", { zlib: { level: 9 } });
  const outFileName = `${import_path.default.basename(dicPath)}.zip`;
  const output = import_fs.default.createWriteStream(import_path.default.resolve(distPath, outFileName));
  const all = convertText(convertDic(dicPath));
  zip.pipe(output);
  zip.append(all, { name: `${import_path.default.basename(dicPath)}.txt` });
  import_fs.default.readdirSync(dicPath).forEach((partLabel) => {
    const partPath = import_path.default.join(dicPath, partLabel);
...
var main = () => {
  const srcPath = import_path.default.resolve("src");
  const distPath = import_path.default.resolve("dist");
  if (!import_fs.default.existsSync(distPath))
    import_fs.default.mkdirSync(distPath);
  const srcDir = import_fs.default.readdirSync(srcPath);
  srcDir.forEach((dicLabel) => {
    const dicPath = import_path.default.join(srcPath, dicLabel);
